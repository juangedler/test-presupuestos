...
class AdministradorController extends WebAppController {
	...
	public function presupuesto(){
		...
		$groups = $this->Group->findAllBygroup_type_id(2, array('id','name','last_percent'));
	...
	public function ceroSaldos(){
		...
		if(date('m') == 12 && !$cero){
			...
			foreach ($balances as $b) {
				...
				$balances_mas_pendings[] = array('Balance' => array('id' => $b['Balance']['id'], 'balance' => 0, 'pending' => 0, 'merchandising' => 0 , 'nacional' => $b['Balance']['balance'] + $b['Balance']['pending'] + $b['Balance']['nacional']));
			}
	...
	public function asignarSaldo(){
		...
		$this->Group->id = $this->request->data['group'];
		...
		try{
		    ...
		    if(!$this->Group->saveField('last_percent', $this->request->data['percentage']))
		        throw new Exception();
	...
	public function anular($in){
		...
		$dataMovement['Movements']['amount'] = $request['Request']['amount'];
		if(isset($request['Request']['mamount']) && $request['Request']['mamount'] != null)
			$dataMovement['Movements']['mamount'] = $request['Request']['mamount'];
		else 
			$request['Request']['mamount'] = 0;
		$dataMovement['Movements']['balance_before'] = $balance['Balance']['balance'];
		$dataMovement['Movements']['created'] = date('Y-m-d H:i:s');
		...
		if(isset($this->request->data['note']) && $this->request->data['note'] != '') 
			$note['RequestNote']['note'] = $this->request->data['note'];
		else 
			$note['RequestNote']['note'] = 'ninguna';
	...
	public function anular_presupuesto_jwt($in){
		$this->layout = null;
		$this->autoRender = false;
		...
		try{
		    ...
		    if(!$this->Request->saveField('current_state_id', 998))
		        throw new Exception();
	...
	public function mostrar($in){
		...
		$query = array('RequestFile.request_id' => $in);
		if($this->request->data['type'] == "3")
			$query = array('RequestFile.request_id' => $in, 'RequestFile.type' => 'Pauta');
		...
		$requestFile['MotivoAnulacion'] = $requestNote;
	...
	public function aprobar_presupuesto($in){
		...
		try{
			...
			$req['request'] = $this->Request->id;
			$req['title'] = $request['Request']['title'];
			$req['note'] = isset($this->request->data['note']) ? $this->request->data['note'] : '';

			try{
				if($next_state == 7){
					$this->sign_pdf($file_path[0]['RequestFile']['file'], $user_sign['User']['signature'], 2);
			    
			    	$datasource->commit();

			    	$this->send_email(3,$req,8);
				}
			    else if($next_state == 8){
					$this->sign_pdf($file_path[0]['RequestFile']['file'], $user_sign['User']['signature'], 3);
				...
				$json['message'] = "¡Ha ocurrido un error! Intente nuevamente.<br>Verifique que su firma este cargada correctamente en el sistema y que sea una imagen de tipo JPG.";
				$jsonstring = json_encode($json);
 				echo $jsonstring;
				return;
			}
	...
	public function aprobar_pauta($in){
		...
		try{
			...
			$objPHPExcel->getActiveSheet()->getColumnDimension($fila)->setWidth(23);
			...
			$objSheet->getStyle($fila.'5')->getFont()->getColor()->setRGB('000080');
			$objSheet->getStyle($fila.'5')->getFont()->setName('Courier')->setBold(true)->setSize(11);
			$objSheet->getCell($fila.'5')->setValue('APROBADO CLIENTE');
	...
	public function modificar_pauta($in){
		...
		$current_state_id = $request['Request']['current_state_id'];
		...
		try{
			...
			if($current_state_id == 12)
			    $this->send_email(0,$req,32);
			else if($current_state_id == 13)
	...
	public function aprobar_presupuesto_ms($role,$in){
		...
		try{
		    ...
		    try{
				...
				if($next_status == 6) {
					$this->sign_pdf($file_path, $user_sign['User']['signature'], 7);
					...
					$this->sign_pdf($file_path, $user_sign['User']['signature'], 8);
	...
	public function modificar_presupuesto_ms($role,$in){
		...
		$obs = (isset($this->request->data['note']) && $this->request->data['note'] != '') ? $this->request->data['note'] : 'ninguna';
		...
		try{
		    ...
		    if(!$this->RequestFile->saveField('status', 8)){
				$this->log("Administrador->modificar_presupuesto_ms: Error modificando registro de la solicitud.", "error");
	    		$this->Session->setFlash("Error modificando registro de la solicitud.");
	        	throw new Exception();
			}

		    if(!$this->RequestFileNote->save($requestFileNote)){
				$this->log("Administrador->modificar_presupuesto_ms: Error guardando observación de la solicitud.", "error");
	    		$this->Session->setFlash("Error guardando observación de la solicitud.");
	        	throw new Exception();
			}
		    
			$req = array(
				'request'	=> $this->RequestFile->id,
			...
			$datasource->rollback();
			$this->log('Administrador->modificar_presupuesto_ms: ' . $e->getMessage(), 'error');
