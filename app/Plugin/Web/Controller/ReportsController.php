...
class ReportsController extends WebAppController {
	...
	public function requestReport(){
		...
		$states = $this->State->find('all', array(
			'fields' => array('State.id','State.process_id','State.name'),
			'recursive'=> -1
		));
	...
	public function availableBalancesReportFilter(){
		...
		*/
		...
		if($end!=NULL && $end!=""){
			$year = date('Y', strtotime($end."23:59:59"));
			$endDate=date('Y-m-d H:i:s', strtotime($end."23:59:59"));
			$currentCondition=array('AND' => array(array('Movement.created <='=> $endDate, 'year(Movement.created) '=> $year)));
			$requestDateCondition = array('Movement.created <= ' =>$endDate, 'year(Movement.created)' => $year);
			$NSMDateCondition = array('NationalSingleMovement.created <= ' =>$endDate, 'year(NationalSingleMovement.created)' => $year);
			$arrayCondition[]=$currentCondition;
		}
		else{
			$year = date('Y');
			$currentCondition=array('AND' => array(array('year(Movement.created) '=> $year)));
			$requestDateCondition = array('year(Movement.created)' => $year);
			$NSMDateCondition = array('year(NationalSingleMovement.created)' => $year);
			$arrayCondition[]=$currentCondition;
		}
		...
		$this->Paginator->settings = array(
			...
			$pending = $this->Movement->find('all', array(
				'conditions' => array(
							"type in ('APROBADA','CERO')",
							$requestDateCondition,
						),
				'fields' => array(
							'Movement.group_id',
							'sum(Movement.amount) as sumPending'
						),
				'group' => 'Movement.group_id',
				'recursive' => -1,
				));

			$pending2 = $this->Movement->find('all', array(
				'conditions' => array(
							"type in ('ANULADA')",
							$requestDateCondition,
						),
			...
			$pending3 = $this->NationalSingleMovement->find('all', array(
				'conditions' => array(
							$NSMDateCondition,
							'type' => 'DEDUCCION'
						),
				'fields' => array(
							'NationalSingleMovement.group_id',
							'sum(NationalSingleMovement.national) as sumPending'
						),
				'group' => 'NationalSingleMovement.group_id',
				'recursive' => -1,
				));

			$pending4 = $this->NationalSingleMovement->find('all', array(
				'conditions' => array(
							$NSMDateCondition,
							'type' => 'REEMBOLSO'
						),
			...
			foreach ($pending as $key => $value) {
				$temp[$value['Movement']['group_id']] = $value[0]['sumPending'];
			}
			...
			foreach ($pending3 as $key => $value) {
				$national_pending[$value['NationalSingleMovement']['group_id']] = $value[0]['sumPending'];
			}

			foreach ($pending4 as $key => $value) {
				$national_pending[$value['NationalSingleMovement']['group_id']] -= $value[0]['sumPending'];
			}
			...
			$this->set(compact('national_pending'));
	...
	public function movementsReportFilter(){
		...
		$currentCondition=array("OR"=>array(
			...
			array('Movement.type LIKE' => "%CERO%"),
			array('Movement.type LIKE' => "%ANULADA%"),
			)
		);
