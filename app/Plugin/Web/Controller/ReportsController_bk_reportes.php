...
class ReportsController extends WebAppController {
	...
	public function availableBalancesReportFilter(){
		...
		if($end!=NULL && $end!=""){
			$endDate=date('Y-m-d H:i:s', strtotime($end."23:59:59"));
			$currentCondition=array('AND' => array(array('HistoricalBalance.created <='=> $endDate)));
			$arrayCondition[]=$currentCondition;
		}
            	...
            	'conditions' => $arrayCondition,
             	'joins' => array(
			        array(
			            ...
			            'type' => 'INNER',
			            'conditions' => array(
			                'Group.id = HistoricalBalance.group_id'
			            )
			        )
				),
	         	'fields'=>array(
	         		'HistoricalBalance.id', 'HistoricalBalance.pending', 'HistoricalBalance.nacional', 'HistoricalBalance.balance', 'HistoricalBalance.created',
			    	'Group.id', 'Group.name', 'Group.city'
				),
        		'recursive'=>-1,
				'limit' => 10, 
             	'order' => array('HistoricalBalance.created' => 'DESC')
				)  
      		);


			$balances=$this->Paginator->paginate('HistoricalBalance');
	...
	public function availableBalancesReportExcel(){
		...
		$balances = $this->HistoricalBalance->find("all", array(
            	'conditions' => $arrayCondition,
             	'joins' => array(
			        array(
			            'table' => 'group',
