...
$('#table_body').on('click', '#consultar', function(){
	...
	$.ajax({
        ...
        success: function(data){
        	...
        	if(<?=$option?> == <?=State::STATE_APROBADO_JWT?>)
    		...
    		$('#modal-body-2').html('<hr><label>Observaciones:</label><textarea type="text" class="requerido observaciones form-control" rows="2"></textarea><br><button type="button" class="btn btn-danger pull-right" data-toggle="modal" data-target="#myModal5" style="margin-right:5px; " value="'+id+'">Anular Solicitud</button>');
...
$('#anular').on('click',function(){
	...
	$.ajax({
		method:'POST',
		data:{note: $('.observaciones').val(),grupoName:$('#concesionario_modal').html()},
		url: "<?php echo $this->Html->url('anular');?>/"+request,
		success: function(data){
			console.log(data);
			$('#loading').fadeOut(0);
        	$('#mensaje_exito').html('<p>Se ha anulado la solicitud con éxito.</p>');
		}
	});
	$('#myModal').modal('hide');
	$('#loading').fadeIn(0);
